...
def check_blame(element, scope=None):
    """Get an element from a config file and check its blame information.

    Args:
        element (str): name of element (e.g. install_tree)
        scope (str): expected config scope to find it in, if not a file
    """
    output = config_cmd('blame', 'config')
    blame_lines = output.rstrip().split('\n')

    element_line = get_config_line(element + ':', blame_lines)
    element = spack.config.get('config:%s' % element)

    if scope is None:
        annotation = '%s:%d' % (element._start_mark.name,
                                element._start_mark.line + 1)
    else:
        annotation = scope

    assert annotation in element_line


def test_config_blame(config):
    """check blame for an element with regular mock config"""
    check_blame('install_tree')
